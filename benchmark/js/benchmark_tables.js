function showSlownessTable(title, results, show_trace_data, category){
    ...
    for( var task in results.task_candidate_wise ){
        ...
        for( var candidate in results.candidate_wise ){
            ...
            if( typeof task_candidates[candidate] != 'undefined' ){
                ...
                var base_candidate_mean = task_candidates[base_candidate].mean;
                
                if( base_candidate_mean == 0 ){
                    base_candidate_mean = task_candidates["caldom"].mean;
                    title = title.replace( base_candidate, "caldom" );
                }
                
                if( task_candidates[base_candidate] ){
                    avg = Math.round( (task_candidates[candidate].mean / base_candidate_mean) * 10 ) / 10;
                }
    ...
    if( task_count > 1 ){
        ...
        for( var candidate in results.candidate_wise ){
            ...
            var avg = Math.round( (candidate_result.mean / base_candidate_result.mean) * 100 ) / 100;
